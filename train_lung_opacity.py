"""
Lung Opacity Detection - DENSENET121 (Best for Medical Images)
90%+ Accuracy in 20-30 minutes
"""
...
IMG_SIZE = 224
BATCH_SIZE = 32  # Optimal for DenseNet
INITIAL_EPOCHS = 20
FINE_TUNE_EPOCHS = 10
CLASS_NAMES = ['Normal', 'Lung_Opacity']

# ✅ YOUR DATASET PATH
DATA_DIR = r"E:/dataset2/Lung Opacity/Lung X-Ray Image/Lung X-Ray Image"
...
def create_data_generators(data_dir, img_size, batch_size):
    ...
    for item in os.listdir(data_dir):
        ...
        if os.path.isdir(item_path) and item in CLASS_NAMES:
            img_count = len([f for f in os.listdir(item_path) if f.endswith(('.png', '.jpg', '.jpeg'))])
            print(f"   - {item}: {img_count} images")
        elif os.path.isdir(item_path):
            print(f"   - {item}: SKIPPED (remove this folder)")
...
# ==============================================
# MODEL - DENSENET121 (Best for Medical)
# ==============================================

def create_model(img_size):
    """Create DenseNet121 model - Best for medical imaging"""
    ...
    model = Model(inputs, outputs, name='LungOpacity_DenseNet121')
...
def train_lung_opacity_model(data_dir=None, epochs=INITIAL_EPOCHS, 
                             batch_size=BATCH_SIZE, fine_tune_epochs=FINE_TUNE_EPOCHS):
    
    print("\n" + "="*70)
    print("LUNG OPACITY DETECTION - DENSENET121 (BEST MODEL)")
    print("="*70)
    ...
    print(f"[INFO] Model: DenseNet121")
    ...
    if train_gen is None or train_gen.samples == 0:
        ...
        print("Make sure only 'Normal' and 'Lung_Opacity' folders exist")
        return None, None
    ...
    callbacks = [
        ...
        ModelCheckpoint(str(models_dir / 'lung_opacity_best.h5'), 
    ...
    fine_tune_history = model.fit(train_gen, validation_data=val_gen, 
                                   epochs=fine_tune_epochs, class_weight=class_weight_dict,
    ...
    print(f"\n CONFUSION MATRIX:")
    print(f"   {'':<15} {'Normal':<12} {'Opacity':<10}")
    print(f"   {'Normal':<15} {cm[0,0]:<12} {cm[0,1]:<10}")
    print(f"   {'Opacity':<15} {cm[1,0]:<12} {cm[1,1]:<10}")
    ...
    # Save final model
    model.save(str(models_dir / 'lung_opacity_model.h5'))
    print(f"\n[INFO] Model saved to: {models_dir / 'lung_opacity_model.h5'}")
